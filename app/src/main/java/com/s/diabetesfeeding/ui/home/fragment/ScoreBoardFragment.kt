...
import com.s.diabetesfeeding.ui.ScoreBoardAdapter
...
class ScoreBoardFragment : Fragment() {
    ...
    private fun showScores(scores: List<ScoreboardData>) {
        ...
        recyclerViewScores.adapter = ScoreBoardAdapter(scores)
    }
    companion object {
        @JvmStatic
        fun newInstance(title: String, bgColorId: Int) =
            ScoreBoardFragment().apply {
                arguments = Bundle().apply {
                    putString(ScoreBoardFragment.ARG_TITLE, title)
                    putInt(ScoreBoardFragment.ARG_BG_COLOR, bgColorId)
                }
            }
        private const val ARG_PARAM1 = "param1"
        private const val ARG_PARAM2 = "param2"
